...
public class SupportChatService : ISupportChatService
{
    ...
    public IEnumerable<SupportChatMessageDto> GetHistory(string roomId, bool senderIsAdmin)
    {
        return _context.Messages
            .Include(x => x.User)
            .Where(x => x.UserId == roomId)
            .Select(x => new SupportChatMessageDto(roomId, x.Content, x.IsFromAdmin ? AdminName : x.User.UserName!, x.SendingDate,
