...
public class RegistrationCommandValidator : AbstractValidator<RegisterCommand>
{
    ...
    private async Task<bool> ValidateExistingEmail(RegisterCommand command, CancellationToken token)
    {
        var checkExistingEmail = await _userManager.FindByEmailAsync(command.Email);
        return checkExistingEmail == null;
    }
    
    private async Task<bool> ValidateExistingUserName(RegisterCommand command, CancellationToken token)
    {
